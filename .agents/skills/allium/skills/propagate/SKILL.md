...
In this repository, run the CLI through `devenv`: `devenv shell -- allium plan <spec>` and `devenv shell -- allium model <spec>`.
...
2. **Read test obligations** — from `allium plan` output (in this repo: `devenv shell -- allium plan <spec>`) or manual derivation
3. **Read domain model** — from `allium model` output (in this repo: `devenv shell -- allium model <spec>`) or manual derivation
