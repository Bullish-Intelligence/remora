...
if entries:
    ...
    for entry in entries:
        summary_lines.append(f"- {entry['name']}: status={entry.get('status', 'unknown')} priority={entry.get('priority', 'none')}")
    summary = '\n'.join(summary_lines)
else:
    summary = 'No workspace entries found yet.'
tool_call = {'name': 'format_summary', 'arguments': {'raw_summary': summary, 'style': style}}
result = {'summary': summary, 'nested_tool_call': tool_call, 'count': len(entries)}
result