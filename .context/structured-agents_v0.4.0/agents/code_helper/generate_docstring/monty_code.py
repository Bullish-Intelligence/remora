...
try:
    ...
    if target_func:
        ...
        if func_end == -1:
            func_end = len(lines)
        func_code = '\n'.join(func_lines[:10])
        result = {'docstring': f'''"""{target_func.split('(')[0].replace('def ', '').replace('async def ', '')} - Auto-generated docstring.\n\nArgs:\n    (See function parameters)\n\nReturns:\n    (Return value description)\n"""''', 'function_detected': target_func, 'code_snippet': func_code}
    else:
        result = {'docstring': '"""Auto-generated docstring.\n\nArgs:\n    (See function parameters)\n\nReturns:\n    (Return value description)\n"""', 'function_detected': None, 'error': 'No function definition found'}
